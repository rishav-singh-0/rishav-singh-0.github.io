...
void *selectFunction(int operationType){
  ...
  void (*operation) = *gOperation[operationType];
  
  return operation;
}

int doOperation(int (*operationFunction)(), int num1, int num2, int * result){

  // validating arguments
  if(NULL==operationFunction){
    printf("[%s]:[%d] Operation Function pointing to NULL\n", __func__, __LINE__);
    return ERR_NULL_FOUND;
  }

  // int error_type = (*gOperation[operationType])(num1, num2, result);
  int error_type = (*operationFunction)(num1, num2, result);
...
int main(){
  ...
  void *operationFunction = selectFunction(operationType);

  int operationErrValue = doOperation(operationFunction, num1, num2, &result);
